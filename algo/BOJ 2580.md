...
- C++
  ...
  bool isRight(vector<vector<int>> &vt);
  vector<int> getPossible(int pos, vector<vector<int>> &vt);
  void getAnswer(vector<vector<int>> &vt);
  ...
  vector<int> getPossible(int pos, vector<vector<int>> &vt){
  	...
  	for (int i = 0; i < 9; i++) {
  		if (vt[row][i] != 0) {
  			rowLine[vt[row][i] - 1]++;
  			if (rowLine[vt[row][i] - 1] > 1) return possible;
  		}
  		if (vt[i][col] != 0) {
  			colLine[vt[i][col] - 1]++;
  			if (colLine[vt[i][col] - 1] > 1) return possible;
  		}
  	}
  	//chk box
  	for (int i = boxRow; i < boxRow + 3; i++) {
  		for (int j = boxCol; j < boxCol + 3; j++) {
  			if (vt[i][j] != 0) {
  				box[vt[i][j] - 1]++;
  				if (box[vt[i][j] - 1] > 1) return possible;
  			}
  		}
  	}
  ...
  void getAnswer(vector<vector<int>> &vt){
  	...
  	for (int i = 0; i < 9; i++) result[i] = vt[i];
  	int pos = -1;
  	while (stk.size() > 0 && result[9][9] != 1) {
  		pos = stk[stk.size() - 1]; stk.pop_back();
  		int row = pos / 10, col = pos % 10;
  		vector<int> possible = getPossible(pos, result);
  		if (possible.size() == 0) {
  			stk.push_back(pos);
  			return;
  		}
  		while (possible.size() > 0 && result[9][9] != 1) {
  			result[row][col] = possible[possible.size() - 1]; possible.pop_back();
  			getAnswer(result);
  		}
  		if (possible.size() == 0 && result[9][9] == 0) {
  			stk.push_back(pos);
  			return;
  		}
  	}
