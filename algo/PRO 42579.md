...
`open addressing`으로 구현시 시간초과, `seperate chaining`으로 구현시 오류. STL `Hashtable`사용 후, `compareTo` 재정의시 부등호 잘못쓴 것을 발견함..

- 오름차순으로 하려면 `this`가 클 때 1을 반환하고, 내림차순으로 하려면 -1을 반환
