...
# URL목록을 만들고, HTML페이지를 읽어오자
url_base <- "https://movie.daum.net/moviedb/grade?movieId=124806&type=netizen&page="
all.reviews <- c()
for(page in 1:604){
  url <- paste(url_base, page, sep="",encoding="euc-kr")
  htxt <- read_html(url)
  table <- html_nodes(htxt, '.review_info')
  cnt_grade <- html_text(html_nodes(table,'.emph_grade'))
  cnt_txt <- str_trim(html_text(html_nodes(table,'.desc_review')))
  all.reviews <- paste(cnt_grade, cnt_txt, sep=", ")
  print(page)
}

# 결과를 파일로 출력하자
write.table(all.reviews, 'kimjiyoung.txt')
